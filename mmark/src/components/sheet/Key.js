...
const Key = ({obj, margin}) => {
    return (
        <div style = {{width: "56px", display: "inline" }}>
            <KeyObj className={obj.key > 0 ? `s-${obj.key}` : obj.key < 0 ? `f${obj.key}`: "0"} margin={margin}/>
        </div>
    );
};
