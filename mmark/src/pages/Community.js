...
class Community extends Component {
    ...
    async componentDidMount() {
        try {
            const res_articles = await fetch('http://3.36.63.218:8000/api/articles');
            const articles = await res_articles.json();
            this.setState({
                articles
            });
            const res_users = await fetch('http://3.36.63.218:8000/api/users');
            const users = await res_users.json();
            this.setState({
                users
            });
            console.log(users);
            const res_tags = await fetch('http://3.36.63.218:8000/api/tags');
